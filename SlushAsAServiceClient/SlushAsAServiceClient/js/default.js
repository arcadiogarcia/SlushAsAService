			...
			if (args.detail.previousExecutionState !== activation.ApplicationExecutionState.terminated) {
				...
				socket.on('event', function (data) {
				    if (data.action == "status") {
				        if (data.status == "stop") {
				            document.getElementById("serverMessage").innerText = "Stopped at " + new Date().toLocaleTimeString();
				        }
				        if (data.status == "start") {
				            document.getElementById("serverMessage").innerText = "Started at " + new Date().toLocaleTimeString()+", will be active "+data.time+" minutes";
				        }
				    }
				});
